...
public:
	...
	double all_norms() const {
		double norms = 0.0;
		for (auto it = p_idxs.begin(); it != p_idxs.end(); ++it) { norms += norm_p(k*it->second); }
		for (auto it = q_idxs.begin(); it != q_idxs.end(); ++it) { norms += norm_q(k*it->second); }
		return norms;
	}
	double norm_q(const int& qi) const { // square of L2 norm 
		double norm = 0.0;
		for (int i = 0; i < k; ++i) { norm += Q[qi+i]*Q[qi+i]; }
		return norm;
	}
	double norm_p(const int& pi) const { // square of L2 norm 
		double norm = 0.0;
...
int main(int argc, char** argv) {
	for (int k = 1; k <= 10;  ++k) {
		Params params(k);
		// sgd("ratings.train.txt", params, k, 0.0, 40, 0.03);
		sgd("ratings.train.txt", params, k, 0.2, 40, 0.03);
		// std::cout << k << ":" << estimate_error("ratings.val.txt", params, k, 0.0) << std::endl;
		std::cout << k << ":" << estimate_error("ratings.val.txt", params, k, 0.2) << std::endl;
	}
	return 0;
}
...
void sgd(const std::string& fname, Params& params, const int& k = 20, const double& lambda = 0.2, const int& iterations = 40, const double& eta = 0.1) {
	...
	// temporary k-dimensional vectors 
	...
	for (int i = 0; i < iterations; ++i) {
		...
		do {
			...
			// assign them to parameters matrices 
	...
	std::cout << "Error for k=" << k << ": " << estimate_error(fname, params, k, lambda) << std::endl;
}
...
double estimate_error(const std::string& fname, Params& params, const int& k, const double& lambda) {
	...
	do {
		...
		crr_movie = atoi(line.substr(t+1, line.find_last_of('\t')).c_str());
		crr_rating = atoi(line.substr(line.find_last_of('\t')+1).c_str());
		if (params.p_idxs.find(crr_user) != params.p_idxs.end() && params.q_idxs.find(crr_movie) != params.q_idxs.end()) {
			p_idx = k*params.p_idxs[crr_user];
			q_idx = k*params.q_idxs[crr_movie];
	...
	infile.close();
	// err += lambda*(params.all_norms()); // error function was redefined for this bullet 
	return err; 
}
